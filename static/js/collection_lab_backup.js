...
async function lookupLabToken() {
  ...
  // Patient details open ONLY when the selected date + token match the same appointment.
  // Date/doctor filters themselves only change the patient/bill list.
  const data = await apiRequest(`/api/lab/patient-by-token?date=${encodeURIComponent(date)}&token=${encodeURIComponent(token)}`);
  if (!data.success) {
    alert(data.message || 'Token Number not found for the selected date.');
    return;
  }
...
async function loadLabBills(){
  ...
  document.getElementById('labBillTotalBills').textContent = data.summary.total_bills;
  document.getElementById('labBillTotalBilling').textContent = `₹${data.summary.total_billing.toFixed(0)}`;
  document.getElementById('labBillPaid').textContent = `₹${data.summary.paid.toFixed(0)}`;
  document.getElementById('labBillPending').textContent = `₹${data.summary.pending.toFixed(0)}`;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(lbd){
    ...
    lbd.addEventListener('change', () => { clearLabPatientView(); loadLabBills(); });
  }
  const lbdoc=document.getElementById('labBillDoctor');
  if(lbdoc) lbdoc.addEventListener('change', () => { clearLabPatientView(); loadLabBills(); });
