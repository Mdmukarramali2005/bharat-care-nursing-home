...
def lab_collection_dashboard(current_user):
    ...
    date_value=request.args.get('date', datetime.date.today().isoformat()); doctor_id=request.args.get('doctor_id',type=int)
    conn=get_db_connection()
    # Lab bills are grouped by the appointment date, not merely by the
    # timestamp on which the bill row was created.
    where="WHERE DATE(COALESCE(a.appointment_date, b.created_at))=?"; params=[date_value]
    ...
    ds=conn.execute(f'''SELECT b.doctor_id,b.doctor_name,COUNT(b.id) bills,
        ...
        FROM lab_bills b LEFT JOIN appointments a ON a.id=b.appointment_id {where} GROUP BY b.doctor_id,b.doctor_name ORDER BY b.doctor_name''',params).fetchall()
...
def lab_patient_by_token(current_user):
    ...
    date_value = (request.args.get('date') or datetime.date.today().isoformat()).strip()
    ...
    appointment = conn.execute("""
        ...
        JOIN doctors d
            ON d.id = a.doctor_id
        WHERE UPPER(a.token_number) = ?
          AND a.appointment_date = ?
        ORDER BY a.id DESC
        LIMIT 1
    """, (
        token_number, date_value,
    )).fetchone()
